...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for Gpos in livingGhostPositionDistributions:
            ...
            if self.distancer.getDistance(pacmanPosition, nextGPos) < closestDist:
                closestDist = self.distancer.getDistance(pacmanPosition, nextGPos)
                closestGhost = nextGPos

        for action in legal:
            CurDist = self.distancer.getDistance(pacmanPosition, closestGhost)
            SuccDist = self.distancer.getDistance(Actions.getSuccessor(pacmanPosition, action), closestGhost)
            if SuccDist < CurDist:
                return action
